...
togglePasswordElements.forEach((toggle) => {
    // Tambahkan event listener untuk setiap ikon mata
    toggle.addEventListener("click", function () {
        // Ambil ID dari input target dari atribut data-target
        const targetId = this.getAttribute("data-target");
        // Cari elemen input password yang sesuai
        const passwordInput = document.getElementById(targetId);
        // Ambil elemen ikon di dalam span
        ...
        // Ganti ikon
        icon.classList.toggle("fa-eye"); // Hapus/tambah ikon mata terbuka
        icon.classList.toggle("fa-eye-slash"); // Tambah/hapus ikon mata tertutup
    });
});
